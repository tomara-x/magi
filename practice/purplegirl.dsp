...
with {
    f(n) = hgroup("[4]t val", par(j,N, nentry("[%2j] %2j",0,-12,12,1))) : ba.selectn(N,n);
    g(n) = hgroup("[5]x mod", par(j,N, nentry("[%2j] %2j",1,-12,12,1))) : ba.selectn(N,n);
    h(n) = hgroup("[6]y mod", par(j,N, nentry("[%2j] %2j",1,-12,12,1))) : ba.selectn(N,n);
    i(n) = hgroup("[7]z mod", par(j,N, nentry("[%2j] %2j",1,-12,12,1))) : ba.selectn(N,n);
};

rat = ba.semi2ratio(index(t,x,y,z)%144);
midc = 261.626;
